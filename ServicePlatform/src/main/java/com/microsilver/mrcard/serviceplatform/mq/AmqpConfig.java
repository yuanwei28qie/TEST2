...
import com.microsilver.mrcard.common.PropertiesUtil;
...
public class AmqpConfig {
	...
	@Bean
    public DirectExchange coordinateExchange() {
        return new DirectExchange(prop.getProperty("rabbitmq.send4.exchange"));
    }
	@Bean
	...
	public Queue coordinateQueue() {
		return new Queue(prop.getProperty("rabbitmq.send4.queue"), true); // 队列持久
	}
	@Bean
	...
	@Bean
	public Binding bindingCoordinate() {
		return BindingBuilder.bind(coordinateQueue()).to(coordinateExchange()).with(prop.getProperty("rabbitmq.send4.routingkey"));
	}
	@Bean
